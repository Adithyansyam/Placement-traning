...
const stats = [
  { icon: ClipboardCheck, label: "Tests Taken", value: "14" },
  { icon: Target, label: "Average Score", value: "72%" },
  { icon: Flame, label: "Study Streak", value: "5 days 🔥" },
  { icon: Gauge, label: "Readiness Score", value: "68%" },
];
  ...
  return (
    <div className="min-h-screen bg-background mesh-bg">
      ...
      <main className="pt-14">
        <div className="max-w-7xl mx-auto px-6 py-8 space-y-6 page-fade">
          {/* Welcome Banner */}
          <motion.div
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
            className="rounded-2xl p-6 text-primary-foreground relative overflow-hidden gradient-bg"
          >
            <div className="absolute top-0 right-0 w-64 h-64 rounded-full bg-white/5 -translate-y-1/2 translate-x-1/4" />
            <div className="absolute bottom-0 left-1/2 w-40 h-40 rounded-full bg-white/5 translate-y-1/2" />
            <div className="absolute top-1/4 right-1/4 w-20 h-20 rounded-full bg-white/[0.03] blur-xl" />
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {stats.map((stat, i) => (
              <motion.div
                ...
                className="glass-card rounded-xl p-5 card-hover border border-border/30"
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl gradient-subtle-bg flex items-center justify-center">
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={scoreData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                    ...
                    <XAxis dataKey="test" tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }} axisLine={{ stroke: "hsl(var(--border))" }} tickLine={false} />
                    <YAxis domain={[40, 100]} tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }} axisLine={{ stroke: "hsl(var(--border))" }} tickLine={false} />
                    <Tooltip
                      contentStyle={{
                    ...
                    />
                    <Line type="monotone" dataKey="score" stroke="hsl(var(--primary))" strokeWidth={2.5} dot={{ r: 4, fill: "hsl(var(--primary))", strokeWidth: 2, stroke: "hsl(var(--card))" }} activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="space-y-3">
                {upcomingTests.map((test) => (
                  <div key={test.name} className="p-4 rounded-xl bg-secondary/50 backdrop-blur-sm border border-border/20 flex items-center justify-between gap-3">
